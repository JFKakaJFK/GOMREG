...
class FCREGBBO_t : public BBOFitnessFunction_t<double>
{
public:
  FCREGBBO_t(int expression_idx,
             vec_t<double> ages,
             vec_t<double> volumes,
             double vtr);
   double objectiveFunction(int objective_index, vec_t<double>& variables);
   double constraintFunction(vec_t<double>& variables);

private:
  int expression_idx;
  vec_t<double> ages;
  vec_t<double> volumes;
  double predict(vec_t<double>& x, double age);
};
